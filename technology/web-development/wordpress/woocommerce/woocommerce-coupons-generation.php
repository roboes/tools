    ...
    function generate_coupon_on_purchase(int|array $input): void
    {
        ...
        foreach ($items_to_process as $item) {
            ...
            if ($coupon_data) {
                ...
                if ($order && $order->get_meta('_coupon_code_' . $coupon_data->meta_suffix)) {
                    continue;
                }
                ...
                if ($order) {
                    ...
                    $order->update_meta_data('_coupon_expiry_timestamp_' . $coupon_data->meta_suffix, $coupon_expiry_date->getTimestamp());
                    $order->save();
                    $order->add_order_note("Coupon created: {$coupon_code}. Valid for: {$product_parent_name}.");
                }

                // Send email
                send_coupon_email(order: $order, product_parent_name: $product_parent_name, product_display_name: $product_display_name, meta_suffix: $coupon_data->meta_suffix, coupon_is_fixed_amount: $coupon_is_fixed_amount, coupon_amount_formatted: $coupon_amount_formatted, coupon_code: $coupon_code, language: $order_language, manual_data: ['name' => $customer_name, 'email' => $customer_email, 'expiry' => $coupon_expiry_date->getTimestamp()]);
            }
        }
    }

    // Send coupon per email
    function send_coupon_email(?WC_Order $order, string $product_parent_name, string $product_display_name, string $meta_suffix, bool $coupon_is_fixed_amount, string $coupon_amount_formatted, string $coupon_code, string $language, array $manual_data = []): void
    {
        ...
        $coupon_expiry_date = DateTimeImmutable::createFromFormat(format: 'U', datetime: (string)$expiry_ts);
        ...
        $email_data = [
            'en' => [
                ...
                'body' => "Hello {$customer_name},<br><br>" . ($order ? "Thank you for your order! Your" : "Your") . " <strong>{$product_display_name}</strong> is now active.<br><br>Gift card code: <strong style='{$email_coupon_code_style}'>{$coupon_code}</strong>" .
                    ...
                    "<br>Valid until: <strong>{$coupon_expiry_date->format(format: get_option('date_format'))}</strong><br><br>Simply use this coupon at checkout for your next booking." . ($coupon_is_fixed_amount ? "<br><br><br><small style='color: #666666;'>Multi-purpose voucher according to Section 3 (15) of the German VAT Act (UStG). VAT is only due upon redemption.</small>" : "")
            ],
            'de' => [
                'subject' => get_option(option: 'blogname') . " - {$product_display_name}",
                'heading' => "{$product_display_name}",
                'body' => "Hallo {$customer_name},<br><br>" . ($order ? "Vielen Dank für deine Bestellung! Dein" : "Dein") . " <strong>{$product_display_name}</strong> ist jetzt aktiv.<br><br>Gutscheincode: <strong style='{$email_coupon_code_style}'>{$coupon_code}</strong>" .
                    ...
                    "<br>Gültig bis: <strong>{$coupon_expiry_date->format(format: get_option('date_format'))}</strong><br><br>Nutze diesen Gutschein einfach bei deiner nächsten Buchung im Warenkorb." . ($coupon_is_fixed_amount ? "<br><br><br><small style='color: #666666;'>Mehrzweckgutschein gemäß § 3 Abs. 15 UStG. Die Umsatzsteuer fällt erst bei Einlösung an.</small>" : "")
            ],
        ];
    ...
    function coupon_balance(int $order_id): void
    {
        ...
        foreach ($coupons_applied as $coupon_code) {
            ...
            if ($coupon->get_id() !== 0 && $coupon->get_discount_type() === 'fixed_cart') {
                ...
                if (!is_array($coupon_redemption_history)) {
                    $coupon_redemption_history = [];
                }

                if (!isset($coupon_redemption_history[$order_id])) {
                    $coupon_redemption_history[$order_id] = [
                        'order_date_created'              => current_time('mysql'),
                        'coupon_amount_redeemed'    => $discount_applied,
                        'coupon_amount_balance_new' => $coupon_new_balance,
                    ];
                    $coupon->update_meta_data('_coupon_redeemed_in_order_ids', $coupon_redemption_history);
                }
