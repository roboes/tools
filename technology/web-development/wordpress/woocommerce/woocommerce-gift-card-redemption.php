...
if (function_exists('WC')) {
    ...
    function send_training_confirmation_email(int $product_id, string $customer_email, string $customer_name, string $product_variation_own_portafilter_machine, string $product_variation_appointment_date, string $product_variation_appointment_time, int $product_quantity, string $language = 'en'): void
    {
        ...
        $messages = [
            ...
            'body' => [
                'en' => 'Hello ' . $customer_name . ',<br><br>You have successfully registered for the following training:<br><br><strong>Training:</strong> ' . (!empty($product_variation_own_portafilter_machine) ? $product_name . ' (Own portafilter machine: ' . $product_variation_own_portafilter_machine . ')' : $product_name) . '<br>' . '<strong>Date:</strong> ' . DateTimeImmutable::createFromFormat(format: 'Y-m-d', datetime: $product_variation_appointment_date, timezone: wp_timezone())->format(get_option(option: 'date_format')) . '<br><strong>Time:</strong> ' . $product_variation_appointment_time . '<br><strong>Quantity:</strong> ' . $product_quantity . '<br><strong>Location:</strong> ' . $product_training_location . ($product_variation_own_portafilter_machine === 'Mit' ? '<br><br><strong>Important:</strong> Please remember to bring your grinder and tamper!<br>' : '') . '<br><br><a href="' . get_permalink($product_id) . '">Product information and legal notice</a><br><br>Thank you for registering!',
                'de' => 'Hallo ' . $customer_name . ',<br><br>Du hast dich erfolgreich für das folgende Training angemeldet:<br><br><strong>Training:</strong> ' . (!empty($product_variation_own_portafilter_machine) ? $product_name . ' (Eigene Siebträgermaschine: ' . $product_variation_own_portafilter_machine . ')' : $product_name) . '<br>' . '<strong>Datum:</strong> ' . DateTimeImmutable::createFromFormat(format: 'Y-m-d', datetime: $product_variation_appointment_date, timezone: wp_timezone())->format(get_option(option: 'date_format')) . '<br><strong>Uhrzeit:</strong> ' . $product_variation_appointment_time . '<br><strong>Menge:</strong> ' . $product_quantity . '<br><strong>Ort:</strong> ' . $product_training_location . ($product_variation_own_portafilter_machine === 'Mit' ? '<br><br><strong>Wichtig:</strong> Bitte bringe unbedingt deine Mühle und deinen Tamper mit!<br>' : '') . '<br><br><a href="' . get_permalink($product_id) . '">Produktinformationen und rechtliche Hinweise</a><br><br>Vielen Dank für deine Anmeldung!',
            ],
        ];
